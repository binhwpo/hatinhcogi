...
@section('content')
  <div class="page-header d-lg-flex d-block">
    <div class="page-leftheader">
        <h4 class="page-title">Quản lý địa điểm</h4>
    </div>
     <div class="page-rightheader ml-md-auto">
            <div class="btn-list mb0">
              <a href="{{ route('place.create') }}">
                <button  class="btn btn-success"> + Thêm địa điểm </button>
              </a>
            </div>
      </div> 
  </div>
              ...
              <tbody>
                @foreach ($places as $item)
                  <tr>
                      ...
                      <td>
                        <div class="d-flex">
                          <a href="{{ route('place.edit', ['place'=>$item->id]) }}" class="action-btns1">
                            <i class="feather feather-edit-2 text-success" data-toggle="tooltip" data-placement="top" title="Sửa bài viết"></i>
                          </a>
                          <a href="{{ route('place.destroy', ['place'=>$item->id]) }}" class="action-btns1" data-toggle="tooltip" data-placement="top" title="Xóa bài viết"><i onclick="return confirm('Bạn có chắc chắn muốn xóa danh mục này không, nếu chắc chắn muốn xóa nhấn vào ok còn chưa chắc chắn nhấn cancel?');" class="feather feather-trash-2 text-danger"></i></a>
                        </div>
                      </td>
                  </tr>
                @endforeach
